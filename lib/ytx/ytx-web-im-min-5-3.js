(function(e) {
    ...
    var YTX_CONFIG = {
        ...
        _onResponse: function(obj) {
            ...
            if (type == YTX_CONFIG._prototype._login) {
                ...
                var msgVersion = loginRsp.version;
                ...
                if (YTX_CONFIG._loginType == 2) {
                    YTX_CONFIG._sessionId = loginRsp._sessionId
                } else {
            ...
            if (type == 2) {
                ...
                for (var i in ipAdds) {
                    ...
                    webSocket.onclose = function(event) {
                        ...
                        if (event.code > 1000) {
                            console.error('ipspeed socket has closed, error code :' + event.code + ': ' + event.reason + ': ' + event.wasClean)
                        }
            ...
            if (!YTX_CONFIG._isConnect) {
                if (!YTX_CONFIG._isConnecting) {
                    ...
                    YTX_CONFIG._socket.onclose = function(event) {
                        if (event.code > 1000) {
                            console.error('websocket has closed, error code :' + event.code + ': ' + event.reason + ': ' + event.wasClean)
                        }
                        var releaseCallBuilder = new RL_YTX.ReleaseCallBuilder();
                        releaseCallBuilder.setCallId(YTX_CONFIG._voipCallData._callEventCallId);
                        releaseCallBuilder.setCaller(YTX_CONFIG._voipCallData._caller);
        ...
        _sendFile: function(fileInfo, content, receiver, msgType, msgId, msgDomain, type, callback, onError, progress, fileName, orignMsgId) {
            ...
            ws.onopen = function(event) {
                ...
                reader.onerror = function(evt) {
                	YTX_CONFIG._log(YTX_CONFIG._logLev._ERROR, 'sendFile reader.onerror code='+evt.target.error.code);
	                if (cnt == 0) {
	                	cnt++;
	                	//重发一次
	                	setTimeout(function(){reader.readAsArrayBuffer(fileInfo);}, 500);
	                } else {
		                var resp = {};
		                resp.code = YTX_CONFIG._errcode._FILE_PARAM_ERROR;
		                resp.msgId = orignMsgId;
		                resp.msgClientNo = msgId;
		                resp.msg = 'file reader error';
		                onError(resp);
		                ws.close()
		                return
	                }
            ...
            ws.onclose = function(event) {
                if (event.code > 1000) {
                    console.error('file client socket has closed, error code :' + event.code + ': ' + event.reason + ': ' + event.wasClean)
                }
                YTX_CONFIG._log(YTX_CONFIG._logLev._INFO, 'file client socket has closed', event.code)
            }
        },
        _heartBeat: function() {
            if (!YTX_CONFIG._isReconnect) {
            ...
            for (var i = 0; i < arrLength; i++) {
                ...
                for (var j = 0; j < bodyArrLength; j++) {
                    if (bodyArr[j].indexOf('rtpmap') > 0) {
                        ...
                        if (bodyArr[j].indexOf('ISAC') > 0 || bodyArr[j].indexOf('G722') > 0 || bodyArr[j].indexOf('rtx') > 0) {
                            delCode = code;
                            continue
                        }
                        newHead += ' ' + code;
        ...
        _sendVoip: function(CallEventBuilder, callback, onError) {
            ...
            function sendOfferAndAnswer(stream) {
                ...
                if (isCaller) {
                    ...
                    var sessionDescription = YTX_CONFIG.util.getSessionDescription();
                    var inviteSdp = YTX_CONFIG._processSDP(YTX_CONFIG._voipCallData._inviteSdp);
    ...
    window.RL_YTX = window.RL_YTX || {
        ...
        login: function(LoginBuilder, callback, onError) {
            ...
            YTX_CONFIG._userAcc = YTX_CONFIG._appid + '_' + LoginBuilder.getUserName();
        ...
        GetTopContactBuilder: function() {},
        SetTopContactBuilder: function() {},
        photo: {
